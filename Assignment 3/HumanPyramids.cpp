...
double whelper(int row,int col,int pyramidHeight,Map<GridLocation,double>&table){
    //double half=0.5;
    if(row<0||col<0){
        error("row&col must be positive!");
    }
    if(col>row){
        error("col should be less than row");
    }
    if(pyramidHeight<row+1){
        error("out of bounds");
    }
    if(row==0&&col==0){
        return 160;
    ...
    //if(table.containsKey({row,col})){
    //    return half*table.get({row,col})+160;
    //}
    else{
        if(col==0&&row>0){
            if(table.containsKey({row-1,0}))
                return table[{row-1,0}]*0.5;
            else
            {   table[{row-1,0}]=whelper(row-1,0,pyramidHeight,table);
                return table[{row-1,0}]*0.5;
            }
        }
        if(col==row){
            if(table.containsKey({row-1,col-1}))
                return table[{row-1,col-1}]*0.5;
            else
            {
                table[{row-1,col-1}]=whelper(row-1,col-1,pyramidHeight,table);
                return table[{row-1,col-1}]*0.5;
            }
        }
        else{
            ...
            if(table.containsKey({row-1,col}))
                sum+=table[{row-1,col}]*0.5;
            else
            {
                table[{row-1,col}]=whelper(row-1,col,pyramidHeight,table);
                sum+=table[{row-1,col}]*0.5;
            }
            sum+=160;
            return sum;
...
double weightOnBackOf(int row, int col, int pyramidHeight) {
    ...
    Map<GridLocation,double> table;
    return whelper(row,col,pyramidHeight,table);
}
