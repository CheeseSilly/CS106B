...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for(const GridLocation &s:sources){
        for(int i=0;i<5;i++){
            int temp_row=s.row+row[i];
            int temp_col=s.col+col[i];
            if(terrain.inBounds(temp_row,temp_col)){
                if(terrain[temp_row][temp_col]<=height){
                    ans[temp_row][temp_col]=true;
                    q.enqueue({temp_row,temp_col});
                }

            }
        }
        while(!q.isEmpty()){
            GridLocation tempGrid=q.dequeue();
            for(int i=0;i<5;i++){
                GridLocation temp(tempGrid.row,tempGrid.col);
                temp.col+=col[i];
                temp.row+=row[i];
                if(terrain.inBounds(temp)){
                    if(terrain[temp]<=height&&ans[temp]==false){
                        ans[temp]=true;
                        q.enqueue(temp);
                    }
