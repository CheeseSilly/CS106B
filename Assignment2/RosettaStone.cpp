...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     */
    if(kGramLength<=0){
         error("kGramLength should not be the negative or zero");
    }
    ...
    for(int i=0;i<str.length();i++){
        string tempStr=str.substr(i,kGramLength);
        if(tempStr.length()<kGramLength){
            continue;
        }
        if(!ans.containsKey(tempStr)){
            ans.put(tempStr,1);
        }
        else{
            ans[tempStr]++;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    for(int i=0;i<numToKeep;i++){
        ...
        ans.put(pq.peek(),source[pq.peek()]);
        pq.dequeue();
    }
    return ans;
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for(string i:lhs){
        for(string j:rhs){
            if(lhs.containsKey(i)&&rhs.containsKey(j)&&i==j){
                ans+=(lhs[i]*rhs[i]);
            }
        }
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for(Corpus i:corpora){
        double temp=cosineSimilarityOf(textProfile,i.profile);
        if(temp>max){
            max=temp;
            ans=i.name;
        }
